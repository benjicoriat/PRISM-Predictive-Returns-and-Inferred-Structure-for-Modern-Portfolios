...
The cost of rebalancing one unit from asset $i$ to asset $j$ at time $t$:
...
combines a fixed asset-class base spread $h_i^0$ (ranging from 1.5\,bps for
US large-cap to 15\,bps for VIX) with a volatility-dependent widening
($\alpha_{\mathrm{tc}}=0.05$), and the market impact
$\eta_i(t) = \beta_{\mathrm{tc}}\hat\sigma_i^{\mathrm{GK}}(t)$
($\beta_{\mathrm{tc}}=0.02$) is proportional to current realised vol.
The matrix is symmetric ($C_{ij}=C_{ji}$), has zero diagonal, and is
\emph{time-varying}: during high-volatility regimes, rebalancing costs
increase automatically, causing the OT solver to prefer smaller trades.
\end{definition}
